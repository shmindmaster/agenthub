...
2. Back up agent configs. AgentHub's fleet synchronizer creates its managed
   ...
   advanced four-host routing mode. Its default `TaskScoped` mode removes stale
   Chrome DevTools registrations rather than installing them.
...
5. Deploy the task-scoped browser policy and remove stale persistent local MCP entries with
   `Sync-AgentHub.ps1 -Apply -Validate -IncludeInactiveAgents -ScopeProfile
    global-default`, then run `Apply-FullAccessAgentProfile.ps1`. Use the
    owning browser skills or a host-native browser plugin during real work. Use
