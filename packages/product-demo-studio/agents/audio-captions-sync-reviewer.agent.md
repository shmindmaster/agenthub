...
Adjudicate the immutable Product Demo Studio envelope against `schemas/candidate-audio-perception-report.schema.json` and its untouched native `ai.ps1 listen` artifact against `schemas/local-ai-listen-report.schema.json`. Confirm `listener.kind` is `local-audio-model`, native execution was local-files-only with no remote inputs, the model id/revision/canonical receipt hash and receipt bytes are bound, the prompt version and raw response bytes are hashed, and a deterministic re-decode covers every exact candidate sample continuously from zero through the declared sample count. Require fresh calibration for the same model receipt hash and prompt with immutable known-good PASS and known-bad FAIL evidence. Confirm the native report evaluated full-program, pronunciation, delivery and pacing, and artifacts and discontinuities. Write the separate `schemas/audio-perception-adjudication.schema.json` artifact in the host output channel. It must bind the exact envelope and candidate bytes plus your host-enforced read-only execution receipt. Do not claim that you, an owner, or another human played or heard the candidate; this is model perception plus independent evidence adjudication. Missing, stale, remote, partial-coverage, failed, or wrong-candidate evidence is `PIPELINE_BLOCKED` for malformed evidence or `REMEDIATE` for a validated audio finding.
