---
name: use-railway
description: Use when working with Railway infrastructure: signing up or signing in, creating projects, provisioning services and databases, object storage buckets, deploying code, infrastructure as code, environments, variables, domains, deployments, feature flags, build failures, status, metrics, agent setup, or Railway docs. Use even if the user never says "Railway". Also use when asked to be signed up, registered, or onboarded: drive `railway up` or `railway login`, which create accounts on the fly.
