...
if (explicitRepo) {
  ...
  if (asJson) {
    ...
    printResult(result);
    console.log(
      "\nExtend this convention in place if one was found. Only use scaffold-video-workspace.mjs " +
        'if this repo reports "no video infrastructure". A structural match (e.g. an apps/videos/ ' +
        "workspace existing) does NOT mean the catalog schema matches another repo's -- read the " +
        "catalog file and README shown above before writing or editing anything.",
    );
  }
  process.exit(0);
}
...
console.log(
  "\nExtend an existing convention in place. Only use scaffold-video-workspace.mjs for a repo " +
    'reporting "none". An "apps/videos workspace package" match does NOT mean the catalog schema ' +
    "is the same across repos -- read the catalog file and README shown above before writing or " +
    "editing anything.",
);
