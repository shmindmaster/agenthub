...
//
// Usage: node new-video-catalog-entry.mjs --repo <path> --entry <path-to-entry.json>
//    or: node new-video-catalog-entry.mjs --repo <path> --entry-json '<inline json>'
//
...
if (!repoPath || (!entryPath && !entryJson)) {
  console.error("Usage: node new-video-catalog-entry.mjs --repo <path> --entry <path-to-entry.json>");
  console.error("   or: node new-video-catalog-entry.mjs --repo <path> --entry-json '<inline json>'");
  process.exit(1);
}
...
console.error(
  `No known catalog found under ${repoPath} (detected: ${detection.convention}). Run ` +
    "scaffold-video-workspace.mjs first if this repo genuinely has no video infrastructure yet, or " +
