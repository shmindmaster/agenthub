...
| SubOps Playwright + FFmpeg | Repository-owned video-capture tests, FFmpeg render scripts, artifact/claim/caption/redaction checks, and guarded seed path | Capture assertions/browser evidence -> evidence package; FFmpeg report -> technical evidence; claims/captions/redaction -> automated accuracy/privacy checks |
| CrewScore release demo | Fixture-driven local app, Playwright capture, Windows narration, render scripts, truth sheet, and claim ledger | Fixtures/state -> synthetic provenance; truth/claim files -> canonical evidence; narration/render outputs -> audio/technical reports |
