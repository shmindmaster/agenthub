...
Do not load retired `product-demo*` skills. Stay in media-studio: `product-picture.md`, `media-studio-capture` (Playwright WebM + Recast via `record-job.mjs`), `media-studio-generate` (local voice; never Motif a screenshot), `validate-product-picture.mjs`, `media-studio-qa`. Job config is `job.json` in the external workspace; a legacy `product-demo-studio.config.yaml` in a product repo is read-only input and must not be created or updated there. All capture and composition code stays external. Engagement is product-picture plus encoded-file review, not `engagement.md` and not a pre-capture text score.
