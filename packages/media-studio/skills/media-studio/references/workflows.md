...
Do not load retired `product-demo*` skills. Stay in media-studio: `product-picture.md` (prove config → action → response → result), capture via `record-job.mjs`, local voice of the locked script, `validate-product-picture.mjs`, `media-studio-qa`. Job config is `job.json` in the external workspace. A legacy in-repo yaml is read-only. Engagement is product-picture plus encoded-file review, not a pre-capture text score.
