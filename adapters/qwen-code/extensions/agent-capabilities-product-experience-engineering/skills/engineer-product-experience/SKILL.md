...
Start from a bare repository. The `_product-experience/` directory is plugin-generated working memory, not an input the user must provide. Apply the bootstrap and reuse rules in [artifact contracts](../../references/artifact-contracts.md). Never stop merely because a prior plugin artifact is absent.
