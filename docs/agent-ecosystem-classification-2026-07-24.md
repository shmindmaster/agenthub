...
| `packages/portfolio-plugins/knowledge-system` | Four host manifests, a retired local ShWiki/RAG MCP reference, and descriptions for a private local corpus | Retained inactive for migration history; local/private-data coupling and no current registry mapping | Preserve unchanged; use `repocontext` as the canonical read-only portfolio-context capability |
...
- RepoContext is the canonical read-only portfolio-context owner. Local stdio is validated; the remote registry entry is not propagated until the private production deployment passes its authenticated contract.
