...
Product Demo Studio release eligibility additionally requires a trusted host-signed execution
receipt and a live deployment report; static packaging never satisfies that gate.
...
| Cursor IDE/CLI | `.cursor-plugin/plugin.json` | Plugin and standalone | Plugin or global MCP; disabled servers do not load | Plugin/custom agents support `readonly: true` | Canonical Cursor manifest now exists, but runtime installation/probing remains prohibited by the active provider hold. Existing loose skills stay in place until a post-hold smoke test. |
...
- A local stdio MCP command is a child process owned by the host/session that launched it.
