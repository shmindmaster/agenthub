...
- The host's own management CLI is a first-party surface and ranks with the rest. On Claude Code, `claude plugin validate` is the authoritative manifest check and `claude plugin disable` disables an enabled plugin without an interactive dialog; other hosts may have their own equivalents. Confirm that a management surface is absent before working around it; assuming one does not exist has been wrong here before.
