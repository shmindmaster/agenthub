...
- Prefer, in order, as a preference rather than a fixed tier list: native repository, shell, test, and version-control tools; then first-party plugins, skills, and MCP servers; then a first-party browser; then first-party GUI control; then direct APIs, SDKs, and CLIs; then specialized automation; then external augmentation. Step down only when the higher preference cannot produce the required outcome.
- The host's own management CLI is a first-party surface and ranks with the rest: `claude plugin validate` is the authoritative manifest check, and `claude plugin disable` removes a plugin without an interactive dialog. Confirm that a management surface is absent before working around it; assuming one does not exist has been wrong here before.
